              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map((log) => (
                  <tr key={log.id} className={log.status === 'failed' ? 'bg-red-50/30' : 'hover:bg-gray-50'}>
                    ...
                    <td className="px-4 py-3 text-gray-700 text-xs max-w-[240px] truncate" title={log.subject ?? ''}>
                      {log.subject ?? '—'}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap">
                    ...
                    <td className="px-4 py-3 text-red-600 text-xs max-w-[200px] truncate" title={log.error_message ?? ''}>
                      {log.error_message ?? '—'}
                    </td>
                  </tr>
                ))}
