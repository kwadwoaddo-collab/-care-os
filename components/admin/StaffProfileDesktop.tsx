...
export default function StaffProfileDesktop({
  ...
  // Activity feed — merge shifts, notes, incidents sorted by date
  type FeedItem = {
            ...
            <div className="p-5">
              ...
              {(dbsExpired || dbsExpiringSoon) && (
                <div className="flex items-start justify-between gap-4 bg-error-container rounded-lg p-4">
                  ...
                  <Link
                    href={`/admin/compliance?staff=${sp.id}`}
                ...
                <div className="divide-y divide-outline-variant">
                  {pendingTraining.map((t) => (
                    <div key={t} className="flex items-center justify-between px-5 py-3">
                      ...
                      <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-yellow-50 text-yellow-700 ring-1 ring-inset ring-yellow-600/20 uppercase tracking-wide">
                        Due Soon
                      </span>
                    </div>
                  ))}
                  {compliance.missingDocuments.slice(0, 2).map((d) => (
                    <div key={d} className="flex items-center justify-between px-5 py-3">
                      ...
                      <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-gray-50 text-gray-600 ring-1 ring-inset ring-gray-400/20 uppercase tracking-wide">
                        Not Started
                      </span>
                    </div>
                  ))}
                  ...
                  <div key={i} className="flex flex-col items-center gap-1.5">
                    ...
                    {day.isOn ? (
                      ...
                      >
                        ...
                        {day.start && (
                          <span className="text-[9px] opacity-80">{day.start}</span>
                        )}
                        {day.end && (
                          <span className="text-[9px] opacity-80">{day.end}</span>
                        )}
                      </div>
                    ) : (
